    ...
    <div class="flex-1 bg-[#F9FAFB] overflow-auto" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto p-4 md:p-8">
            ...
            <!-- Grid View -->
            <div x-show="viewMode === 'grid'" class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                ...
                @forelse($notes as $note)
                    ...
                    <div 
                        ...
                        class="rounded-xl border border-gray-200 group hover:shadow-lg transition-all overflow-hidden flex flex-col"
                        :class="selectedNotes.includes({{ $note->id }}) ? 'ring-2 ring-blue-500 border-blue-500' : ''"
                        :style="selectedNotes.includes({{ $note->id }}) ? 'background-color: white;' : 'background-color: {{ $bgColor ?: 'rgba(148, 163, 184, 0.08)' }};'"
                    >
                        <!-- Header: Title + Star + Checkbox -->
                        <div class="px-4 pt-4 pb-2">
                            <div class="flex items-start justify-between gap-3 mb-2">
                                <h3 class="text-base font-bold text-[#1E293B] line-clamp-1 flex-1 group-hover:text-[#2C74B3] transition-colors cursor-pointer"
                                    @click="window.location='{{ route('notes.show', $note) }}'">
                                    {{ $note->title }}
                                </h3>
                                
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    <button 
                                        @click.stop="toggleFavorite({{ $note->id }}, $event)"
                        ...
                        <div class="px-4 pb-3 cursor-pointer flex-1"
                            @click="window.location='{{ route('notes.show', $note) }}'">
                        ...
                        <div class="px-4 py-3 border-t border-gray-200/50">
                            <div class="flex items-center justify-between gap-2 text-xs">
                                @if($note->category)
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-1 text-white font-medium rounded-md {{ $class }}" @if($style) style="{{ $style }}" @endif>
                                        {{ $note->category->name }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 bg-gray-200 text-gray-600 font-medium rounded-md">
                                        Tanpa Kategori
                                    </span>
                                @endif
                        ...
                        <div class="px-4 py-2 bg-gray-50/50 border-t border-gray-200/50">
                            <div class="flex items-center justify-center gap-1">
                                <!-- View Button -->
                                <a href="{{ route('notes.show', $note) }}" 
                                @click.stop
                                class="p-2 hover:bg-white rounded-lg transition-colors text-gray-500 hover:text-[#2C74B3]"
                                title="Lihat">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                class="p-2 hover:bg-white rounded-lg transition-colors text-gray-500 hover:text-green-600"
                                title="Share">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('notes.destroy', $note) }}" method="POST" @click.stop class="inline">
                                    ...
                                    <button type="submit" 
                                            ...
                                            class="p-2 hover:bg-white rounded-lg transition-colors text-gray-500 hover:text-red-600"
                                            title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                ...
                @forelse($notes as $note)
                    ...
                    >
                        ...
                        <div class="cursor-pointer pl-12 pr-12" @click="if (!$event.target.classList.contains('note-checkbox')) { window.location='{{ route('notes.show', $note) }}' }">
                            ...
                            <!-- Footer: Category & Date -->
                            <div class="flex items-center justify-between gap-1 sm:gap-2 text-xs">
                                @if($note->category)
                                    @php
                                        $catColor = $note->category->color;
                                        $style = str_starts_with($catColor, '#') ? "background-color: {$catColor};" : '';
